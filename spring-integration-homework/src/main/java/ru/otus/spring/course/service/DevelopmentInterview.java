...
@Service
@Slf4j
public class DevelopmentInterview implements Interview {
    @Override
    public Scoring interview(Portfolio portfolio) {
        int points = score();
        log.info(String.format("Candidate %s %s received %d points from %s", portfolio.getName(), portfolio.getSurname(), points, DevelopmentInterview.class));
